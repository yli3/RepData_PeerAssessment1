...
`Total steps per day` has mean $\sim$ ``r mean(dt.total$totalSteps)`` and median ``r median(dt.total$totalSteps)``.

## Average daily activity pattern 

Next, we'll consider the average daily activity pattern -- that is, for each of the ``r 24 * 60 / 5`` five-minute intervals of the day, what was the average number of steps taken over the whole of the dataset? As before, we'll continue to ignore `NA` values.
...
We find the 5-minute interval with the highest average steps over this dataset to be the one`r ifelse(length(dt.daily.max >1),"s","")` starting at ``r dt.daily.max.output``, with $\sim$ ``r round(max(dt.daily$avgSteps), 1)`` average steps.

## Imputing missing values

We now turn to the question of missing values. In the original dataset, there were ``r sum(is.na(dt))`` `NA` values. By omitting these from calculations, we may have introduced biases into our results.
...
```{r na_impute_method, echo = TRUE, results = "hide"}
...
```{r na_verify_weekdays, echo = TRUE}
...
```{r na_interval_averages, echo = TRUE}
  na.omit(dt)[, length(unique(date)), by = dayWeek]
```

We'll now revisit our first question - *what is the total number of steps taken per day?* - and evaluate it with our newly-completed dataset `dt.complete` (which now has ``r sum(is.na(dt.complete))`` `NA` rows).
...
Now `Total steps per day` has mean $\sim$ ``r mean(dt.complete.total$totalSteps)`` and median ``r median(dt.complete.total$totalSteps)``. To compare, the original results from the NA-omitted dataset were mean ``r mean(dt.total$totalSteps)`` and median ``r median(dt.total$totalSteps)``.
...
```{r dayType_factor, echo = TRUE, results = "hide"}
...
```{r daily_activity_complete_dayType}
  ...
  ggplot(data = dt.complete.daily, aes(x = interval, y = avgSteps)) +
    ...
    labs(title = "Completed Daily Activity, Weekends and Weekdays\n") +
    labs(x = "\nInterval Start Time", y = "Average steps per interval\n") +
    scale_x_continuous(
      breaks = seq(0, 24 * 60, 180),
      minor_breaks = seq(0, 24 * 60, 60),
...
```{r complete_mean_dayType, echo = TRUE}
