...
A simple way to fill in the missing values is to use the mean interval value over the dataset. For our dataset, this would add ``r length(unique(dt$date)) - length(unique(dt[naRows]$date))`` identical days that follow the overall mean activity pattern. The overall mean would be preserved exactly, and the median would be nudged toward, if not reach, the mean.
