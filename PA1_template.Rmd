...
`Total steps per day` for `dt.complete` has mean ``r mean(dt.complete.total$totalSteps)`` and median ``r median(dt.complete.total$totalSteps)``. To compare, the original results from the NA-omitted dataset were mean ``r mean(dt.total$totalSteps)`` and median ``r median(dt.total$totalSteps)``.
